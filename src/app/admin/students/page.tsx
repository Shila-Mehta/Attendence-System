                ...
                <tr key={r.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="leading-tight">
                      ...
                      <div className="text-xs text-muted-foreground">{r.guardianPhone}</div>
                    </div>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
...
function StatusBadge({ status }: { status: StudentStatus }) {
  const cls =
    status === "Active" ? "bg-success-light text-success" : "bg-danger-light text-danger";
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg rounded-lg bg-surface border border-border shadow-xl">
        <div className="flex items-center justify-between px-5 h-14 border-b border-border">
          <h2 className="text-sm font-semibold">{isEdit ? "Edit Student" : "Add Student"}</h2>
        ...
        <div className="p-5 grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Full name">
            <input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="e.g. Ayesha Khan" className={inputCls} />
          </Field>
          <Field label="Grade">
          ...
          <Field label="Class">
            <select value={form.class} onChange={(e) => set("class", e.target.value)} className={inputCls}>
              {["Class A","Class B","Class C"].map((c) => <option key={c}>{c}</option>)}
            </select>
          </Field>
          ...
          <Field label="Guardian phone">
            <input value={form.guardianPhone} onChange={(e) => set("guardianPhone", e.target.value)} placeholder="+92 300 0000000" className={inputCls} />
          </Field>
          <Field label="Status">
            <select value={form.status} onChange={(e) => set("status", e.target.value as StudentStatus)} className={inputCls}>
              <option value="Active">Active</option>
              <option value="Inactive">Inactive</option>
            </select>
          </Field>
          <Field label="Enrolled at">
            <input type="date" value={form.enrolledAt} onChange={(e) => set("enrolledAt", e.target.value)} className={inputCls} />
          </Field>
        </div>
