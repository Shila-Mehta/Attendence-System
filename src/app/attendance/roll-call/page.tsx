...
export default function RollCallPage() {
  ...
  const className = `${currentClass.grade} · ${currentClass.name}`;
  const hasAnyMarks = counts.present + counts.absent + counts.late > 0;
  ...
  const markAllPresent = () => {
    const toMark = students.filter((s) => marks[s.id] !== "Present").length;
    setMarks((m) => {
      const next = { ...m };
    ...
    pushToast(
      "success",
      "All marked present",
      `${toMark} student${toMark === 1 ? "" : "s"} updated in ${className}.`
    );
  };

  const doClear = () => {
    const clearedCount = hasAnyMarks
    ...
    pushToast(
      ...
      "Marks cleared",
      clearedCount > 0
        ? `${clearedCount} mark${clearedCount === 1 ? "" : "s"} cleared in ${className}.`
        : `No marks to clear in ${className}.`
    );
  };

  const canSubmit = counts.unmarked === 0 && !submitted && students.length > 0;
  ...
  const progressPct = counts.total
    ? Math.round(((counts.total - counts.unmarked) / counts.total) * 100)
    : 0;
      ...
      actions={
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className="h-10 sm:h-9 px-3 rounded-md border border-border text-sm font-medium sm:font-normal hover:bg-muted inline-flex items-center justify-center gap-1.5 flex-1 sm:flex-none transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
    ...
    >
      ...
      {loading ? (
        <div className="rounded-lg border border-border bg-surface">
          <LoadingState
            title="Loading roll call…"
            description="Fetching the class list and students."
          />
        </div>
      ) : error ? (
        /* ================= ERROR (Phase 10) ================= */
        ...
        <>
          ...
          {students.length === 0 ? (
            <div className="rounded-lg border border-border bg-surface">
              <EmptyState
                ...
                description="Once students are assigned to this class, they'll appear here for roll call."
              />
            </div>
          ) : (
            <>
          ...
          <ConfirmDialog
            ...
            message={
              <>
                This will remove all attendance marks for{" "}
                <span className="font-medium text-foreground">{className}</span>
                . You&apos;ll need to mark each student again. This action
                cannot be undone.
              </>
            }
